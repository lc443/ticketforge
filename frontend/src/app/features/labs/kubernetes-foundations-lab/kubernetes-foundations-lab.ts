...
@Component({
  selector: 'app-kubernetes-foundations-lab',
  standalone: true,
  imports: [LabBreadcrumb, ScenarioCard],
  templateUrl: './kubernetes-foundations-lab.html',
  styleUrl: './kubernetes-foundations-lab.scss',
})
export class KubernetesFoundationsLab {
  readonly exerciseCount = 6;
  readonly completed = signal<Set<number>>(new Set());
  readonly answer = signal<string | null>(null);
  readonly progress = computed(() =>
    Math.round((this.completed().size / this.exerciseCount) * 100),
  );
